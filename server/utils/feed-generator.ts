...
function buildFeed(
  ...
  const faviconUrl = baseUrl ? `${baseUrl}/favicon.svg` : undefined;
 ...
 */
export function generateRssXml(feed: ExtractedFeed, selfUrl: string, baseUrl?: string): string {
  const f = buildFeed(feed, selfUrl, "rss", baseUrl);
 ...
 */
export function generateAtomXml(feed: ExtractedFeed, selfUrl: string, baseUrl?: string): string {
  const f = buildFeed(feed, selfUrl, "atom", baseUrl);
